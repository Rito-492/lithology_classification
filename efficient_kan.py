...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        self.register_buffer("grid", grid.to(self.device))
        ...
        if enable_standalone_scale_spline:
            self.spline_scaler = torch.nn.Parameter(
                torch.Tensor(out_features, in_features).to(self.device)
            )
    ...
    def to(self, device):
        self.device = device
        super().to(device)
        self.grid = self.grid.to(device)  # 迁移buffer
        return self
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.reshape(-1, self.in_features).to(self.device)
